...
import queries
from optimization import get_implication_graph, batch_cfs_generation

PredictionsExample = collections.namedtuple("PredictionsExample", ["prediction_id", "instance_id", "z3_expression", "constraints", "label", "view_id","cf_type","classifier_id"])
...
class CFQL:
    ...
    def add_view(self, cf_type: str, prediction_query: str, cfs_query, verbose):
    ...
    def batch_execute(self, query, metric, threshold, hashing=True, atom_implication=True, max_CF_to_check=None):
        views = {}
        def add_value_to_key(key, value):
            if key not in views:
                views[key] = []
            views[key].append(value)

        all_expressions = self.get_expressions(query)
        all_views = set(e.view_id for e in all_expressions)
        field_indices = {field: index for index, field in enumerate(PredictionsExample._fields)}

        grouped_predictions = {key: list(group) for key, group in groupby(all_expressions,
                                                                          key=itemgetter(field_indices["cf_type"],
                                                                                         field_indices["classifier_id"],
                                                                                         field_indices["label"]))}
        graphs = {}
        # Loop over the groups
        for (cf_type, classifier_id, label), expressions in grouped_predictions.items():
            if cf_type == "DiverseCFs":
                raise Exception("Sorry, optimization is not yet supported for DiverseCFs")

            encoder = self.models[classifier_id][1]
            cf_alg = self.CF_generations[cf_type][classifier_id]
            implication_graph = get_implication_graph(encoder, expressions, atom_implication, hashing)
            if hashing and atom_implication:
                implication_graph = batch_cfs_generation(implication_graph, metric, threshold, self, cf_alg , max_CF_to_check)
            graphs[(cf_type, classifier_id, label)] = implication_graph

        for (cf_type, classifier_id, label), implication_graph in graphs.items():
            cf_alg = self.CF_generations[cf_type][classifier_id]
            for equivalence_class in implication_graph.nodes:
                for node in implication_graph.nodes[equivalence_class]['members']:
                    view_id, prediction_id = node
                    cf_id = implication_graph.graph['Ans'].get(node)
                    if cf_id is not None:
                        full_cf = implication_graph.graph['CFs'][cf_id]
                        instance = self.instances.get_instance(implication_graph.graph['id2expressions'][node].instance_id)
                        instance = instance.reset_index().drop(columns=['InstanceId'])
                        ne_stacked = (instance != full_cf).stack()
                        changed = ne_stacked[ne_stacked]
                        difference_locations = np.where(instance != full_cf)
                        changed_to = full_cf.values[difference_locations]
                        cfs = pd.DataFrame({'NewValue': changed_to}, index=changed.index)
                        cfs.index.rename(['CfId', 'FeatureName'], inplace=True)
                        cfs.reset_index(level=1, inplace=True)
                        cfs.reset_index(inplace=True)
                        output = self.insert_to_db(cf_alg, instance, cfs, prediction_id)
                        add_value_to_key(view_id, output)

        for view_id, dfs in views.items():
            view = self.cf_views[view_id]
            cfs, prediction_cfs = list(map(list, zip(*dfs)))
            cfs = pd.concat(cfs, ignore_index=True)

            cfs.to_sql(view.get_cf_name(), self.db, if_exists='append', index=False)
            prediction_cfs = view.get_prediction_cf_from_db()
            prediction_cfs.to_sql(view.get_prediction_cf_name(), self.db, if_exists='append', index=False)
            view.cfs = cfs
            view.prediction_cfs = prediction_cfs

            view.is_computed = True
        if set(views.keys()) != all_views:
            raise Exception(f"Sorry, unable to calculate {all_views - set(views.keys())} views.")
    ...
    def create_cfs_view(self, cf_type: str, prediction_query: str, cfs_query=None, verbose=True):
        if cf_type not in ['DiverseCFs', 'CecCFs', 'GrowingSpheresCFs']:
            raise NotImplementedError(f'{cf_type} not implement yet')
        else:
            self.add_view(cf_type, prediction_query, cfs_query, verbose)
...
class CFsView:
    ...
    def __init__(self, querylang: CFQL, cf_type: str, prediction_query: str, cfs_query, processes=2):
        self.processes = processes
