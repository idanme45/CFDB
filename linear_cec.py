...
class LinearCec:
    ...
    def get_cfs(self, instance, to_class, conditions):
        ...
        for i in range(self.max_iter):
            ...
            preds = self.clf.predict(cf)
            matching_indices = np.where(preds == to_class)[0]
            if len(matching_indices) > 0:
                break
        return len(matching_indices) > 0, cf.iloc[matching_indices[0]:matching_indices[0]+1] if len(matching_indices) > 0 else None
